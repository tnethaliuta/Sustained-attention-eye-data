...
def get_feature_matrix(df):
    ...
    no_of_cols = len(Feature_matrix.columns)+1
    Feature_matrix.columns = list(range(1,no_of_cols))
    #renaming the index
...
#First create a df with the median values for each problem's accuracy
    ...
    for k in range(6):
        ...
        if max_feats_for_prob == 0:
            summary_mean_acc_df1 = summary_mean_acc_df[cols[k]].drop(0) #make a new df by dropping the 0th row.
            max_accu_for_prob = summary_mean_acc_df1.max() #max of the problem
            #threshold = max_acc_for_prob-0.05
            #best_feat_indices = np.where(summary_mean_acc_df[cols[k]] >= threshold)[0].tolist()
            max_feats_for_prob = summary_mean_acc_df1.idxmax()
        else:
